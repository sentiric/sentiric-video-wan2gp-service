import boto3, aio_pika, structlog, os
...
class S3Uploader:
    ...
    def upload_file(self, file_path, job_id, trace_id):
        obj_name = f"videos/{job_id}.mp4"
        self.s3.upload_file(file_path, settings.S3_BUCKET, obj_name)
        s3_uri = f"s3://{settings.S3_BUCKET}/{obj_name}"
        logger.info("Video file uploaded to S3", event_id="S3_UPLOAD_SUCCESS", trace_id=trace_id, uri=s3_uri)
        return s3_uri
        ...
        try:
            ...
            async with conn:
                ...
                msg = aio_pika.Message(
                    body=evt.SerializeToString(),
                    content_type="application/protobuf",
                    delivery_mode=aio_pika.DeliveryMode.PERSISTENT
                )
                
                await ex.publish(msg, routing_key=event_type)
                logger.info(f"Event published: {event_type}", event_id="RMQ_PUBLISH_SUCCESS", trace_id=trace_id)
